-- GOLD SOURCE (PF-152): a real migration from this project, copied verbatim, demonstrating the
-- double-underscore naming convention, FOREIGN KEY constraints (with a validation-first ALTER
-- TABLE ADD CONSTRAINT pattern, not inline at CREATE TABLE time), CREATE INDEX including partial
-- indexes with a WHERE clause, and COMMENT ON TABLE documentation.
--
-- Does NOT demonstrate every pattern flyway-migration-architect's own Gotchas cover, and doesn't
-- need to duplicate them here: TIMESTAMPTZ only became the convention in V28 (this migration
-- predates it and uses plain TIMESTAMP, matching its own real point in history -- copy the
-- TIMESTAMPTZ convention from a post-V28 migration, not from this file); this project has no
-- money-column example here (real convention is NUMERIC(19,2), see the Gotchas); UUID does not
-- appear anywhere in this codebase's migrations -- every table uses a natural VARCHAR/CHAR key or
-- BIGSERIAL, never UUID. Don't introduce UUID on the assumption it's this project's convention.
--
-- Original file: V26__optimize_accounts_indexes_and_lookup_tables.sql
